...
def render_trend_analytics() -> None:
    ...
    st.markdown(
        "Compare analysis patterns across sessions, detect drift, and track KPIs over time."
    )
    ...
    with tab1:
        ...
        with st.expander("📅 Session Details"):
            for s in sessions:
                st.markdown(
                    f"""<div style="background:#161b22;border-radius:8px;padding:12px;margin:4px 0;border-left:3px solid {"#ff4444" if s["spam_rate"] > 0.5 else "#44bb44"}">
        ...
        if len(sessions) >= 2:
            ...
            with col1:
                s1_idx = st.selectbox(
                    ...
                    format_func=lambda i: (
                        f"Session {i} ({sessions[i]['start'].strftime('%b %d %I:%M %p') if hasattr(sessions[i]['start'], 'strftime') else sessions[i]['start']})"
                    ),
                    key="s1",
                )
            with col2:
            ...
            metrics = [
                (
                    ...
                    s1["total"],
                    s2["total"],
                    f"{s2['total'] - s1['total']:+d}",
                ),
                (
                    "Spam Rate",
                    f"{s1['spam_rate']:.1%}",
                    f"{s2['spam_rate']:.1%}",
                    f"{s2['spam_rate'] - s1['spam_rate']:+.1%}",
                ),
                (
                    "Avg Confidence",
                    f"{s1['avg_confidence']:.1%}",
                    f"{s2['avg_confidence']:.1%}",
                    f"{s2['avg_confidence'] - s1['avg_confidence']:+.1%}",
                ),
                (
                    "Duration",
                    f"{s1['duration_seconds'] / 60:.1f}m",
                    f"{s2['duration_seconds'] / 60:.1f}m",
                    f"{s2['duration_seconds'] - s1['duration_seconds']:+.0f}s",
                ),
                (
                    "Throughput",
                    f"{s1['throughput_per_hour']:.0f}/hr",
                    f"{s2['throughput_per_hour']:.0f}/hr",
                    f"{s2['throughput_per_hour'] - s1['throughput_per_hour']:+.0f}/hr",
                ),
                (
                    "Models Used",
                    s1["model_count"],
                    s2["model_count"],
                    f"{s2['model_count'] - s1['model_count']:+d}",
                ),
            ]
            st.markdown("### 📊 Side-by-Side Comparison")
            st.markdown(
                f"""<div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:8px;font-size:0.85rem">
    ...
    with tab3:
        if drift_data["deltas"]:
            ...
            for d in drift_data["deltas"]:
                ...
                st.markdown(
                    f"""<div style="background:#161b22;border-radius:8px;padding:10px;margin:3px 0">
    ...
    with tab4:
        ...
        st.download_button(
            ...
            csv,
            f"session_analytics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            "text/csv",
            use_container_width=True,
        )
        if drift_data["deltas"]:
            drift_json = pd.DataFrame(drift_data["deltas"]).to_json(
