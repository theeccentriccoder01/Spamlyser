...
def test_export_onnx_creates_file(tmp_path):
    ...
    optimize_model.torch.onnx.export.side_effect = (
        lambda _model, _dummy, path, **_kwargs: Path(path).touch()
    )
...
def test_export_torchscript_creates_file(tmp_path):
    ...
    traced = optimize_model.torch.jit.trace.return_value
    traced.save.side_effect = lambda path: Path(path).touch()
