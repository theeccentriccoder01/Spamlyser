        ...
        for model_name in ["DistilBERT", "BERT", "RoBERTa", "ALBERT"]:
            try:
                ...
                predictions[model_name] = {
                    ...
                    "spam_probability": 0.0, "error": str(e),
                }
...
def render_what_if_playground(classifier) -> None:
    ...
    st.markdown("Edit the message text and see how each model's prediction changes in real time.")

    analyzer = WhatIfAnalyzer()
    if "whatif_text" not in st.session_state:
        st.session_state.whatif_text = "Your account has been compromised! Click here to verify."
    ...
    with col1:
        ...
        templates = [
            ("📧 Phishing", "Dear customer, your Netflix account has been suspended. Verify now: http://bit.ly/netflix-verify"),
            ("💰 Prize Scam", "CONGRATULATIONS! You've won $1,000,000 in the international lottery! Call +1234567890 now to claim!"),
            ("👋 Personal", "Hey! Are we still meeting for coffee at 3pm tomorrow?"),
            ("🏪 Promo", "Exclusive 50% off at Walmart! Shop now before the offer ends!"),
        ]
    ...
    with col2:
        ...
        st.metric("Ensemble Label", result["ensemble_label"],
    ...
    for idx, (model_name, pred) in enumerate(models_display):
        with cols[idx]:
            ...
            label_color = "#ff4444" if pred["label"] == "SPAM" else ("#44bb44" if pred["label"] == "HAM" else "#ffa657")
            label_icon = "🔴" if pred["label"] == "SPAM" else ("🟢" if pred["label"] == "HAM" else "⚪")

            st.markdown(f"""<div class="whatif-model-card">
            ...
            if delta_info:
                arrow = "▲" if delta_info["direction"] == "up" else ("▼" if delta_info["direction"] == "down" else "—")
                arrow_class = "whatif-delta-up" if delta_info["direction"] == "up" else ("whatif-delta-down" if delta_info["direction"] == "down" else "")
                st.markdown(f"""<div style="margin-top:6px;padding-top:6px;border-top:1px solid #30363d">
    ...
    if words:
        ...
        st.markdown(f'<div style="font-size:1.1rem;line-height:2">{impact_html}</div>', unsafe_allow_html=True)
