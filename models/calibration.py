...
class ConfidenceCalibrator:
    ...
    def fit_temperature(self, y_true: np.ndarray, y_prob: np.ndarray) -> float:
        ...
        def nll_loss(t):
            ...
            loss = -np.mean(y_true * np.log(scaled_probs) + (1.0 - y_true) * np.log(1.0 - scaled_probs))
            return loss
    ...
    def generate_calibration_curve(self, y_true: np.ndarray, y_prob: np.ndarray, n_bins: int = 10) -> Dict[str, Any]:
