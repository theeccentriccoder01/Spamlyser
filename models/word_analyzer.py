...
class WordAnalyzer:
    ...
    def analyze_text(self, text: str) -> dict[str, Any]:
        ...
        for i, word in enumerate(words):
            ...
            context_multiplier = self._get_context_multiplier(
                word, prev_word, next_word, context
            )
            ...
            is_influential = total_influence > 0.3 or (
                spam_weight > 0.5 or ham_weight > 0.5
            )
        ...
        predicted_class, confidence = self._determine_prediction(
            spam_ratio, ham_ratio, total_score, complexity_factor
        )
    ...
    def _get_context_multiplier(
        self, word: str, prev_word: str, next_word: str, context: str
    ) -> float:
        ...
        for i, word in enumerate(words):
            if i < len(analysis["words"]):
                ...
                if influence > 0 or fallback_ham:
                    ...
                    base_influence = (
                        0.15 if fallback_ham and influence == 0 else influence
                    )
    ...
    def get_explanation_summary(self, analysis: dict[str, Any]) -> dict[str, Any]:
        ...
        suspicious_words = [
            w for w in influential_words if w["word_type"] == "suspicious"
        ]

        # Calculate additional metrics
        high_influence_words = [
            w for w in influential_words if w["total_influence"] > 0.7
        ]
        medium_influence_words = [
            w for w in influential_words if 0.3 < w["total_influence"] <= 0.7
        ]
        low_influence_words = [
            w for w in influential_words if 0 < w["total_influence"] <= 0.3
        ]
        ...
        display_ham_words = [
            {
                ...
                "influence": -w.get(
                    "ham_weight", 0.1
                ),  # Negative influence for ham words
